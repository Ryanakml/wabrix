    ...
    <Drawer open={open} onOpenChange={onOpenChange} direction='right'>
      <DrawerContent className='data-[vaul-drawer-direction=right]:w-full data-[vaul-drawer-direction=right]:sm:max-w-md'>
        <DrawerHeader className='border-border/50 border-b px-5 py-4'>
          <DrawerTitle>{conversation ? `${conversation.name} details` : 'Conversation details'}</DrawerTitle>
          <DrawerDescription>
            Operational metadata and actions are kept here to preserve a clean message thread.
          </DrawerDescription>
        </DrawerHeader>

        <div className='min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4'>
          {isLoading ? (
            <p className='text-muted-foreground text-sm'>Loading conversation details…</p>
          ) : !selectedConversation ? (
            <p className='text-muted-foreground text-sm'>Select a conversation to view details.</p>
          ) : (
            <>
              <Section title='Conversation Metadata'>
                <Row label='Service Window' value={selectedConversation.serviceWindowOpen ? 'Open' : 'Closed'} />
                <Row
                  label='Reply Policy'
                  value={selectedConversation.replyPolicy === 'freeform' ? 'Freeform replies allowed' : 'Template-only replies'}
                />
                <Row
                  label='Service Window Expires'
                  value={selectedConversation.serviceWindowExpiresAt ?? 'Not available'}
                />
                <Row label='Last Inbound' value={selectedConversation.lastInboundAt} />
                <Row label='Last Message' value={selectedConversation.lastMessageAt} />
              </Section>

              <Section title='Bot State & Errors'>
                <Row label='Bot Reply State' value={selectedConversation.botReplyState} />
                <Row label='Bot Paused' value={selectedConversation.botPaused ? 'Yes' : 'No'} />
                <Row
                  label='Bot Error'
                  value={selectedConversation.botReplyError ?? 'No active bot errors'}
                />
              </Section>
              ...
              <Section title='Assignment & Actions'>
                ...
                <div className='grid gap-2 sm:grid-cols-2'>
                  ...
                  >
                    {selectedConversation.botPaused ? 'Resume Bot' : 'Pause Bot'}
                  </Button>
                  <Button
                    type='button'
                    variant='outline'
                    ...
                    isLoading={isUpdatingConversation}
                  >
                    {selectedConversation.handoffRequested ? 'Clear Handoff' : 'Request Handoff'}
                  </Button>
                  <Button
                    type='button'
                    variant='outline'
              ...
              <Section title='WABA Lifecycle'>
                <Row label='Connection' value={details?.wabaLifecycle.connectionStatus ?? 'Unknown'} />
                <Row label='Webhook' value={details?.wabaLifecycle.webhookStatus ?? 'Unknown'} />
                <Row label='Approval' value={details?.wabaLifecycle.approvalStatus ?? 'Unknown'} />
                <Row label='OTP' value={details?.wabaLifecycle.otpStatus ?? 'Unknown'} />
                <Row label='Profile Sync' value={details?.wabaLifecycle.profileSyncStatus ?? 'Unknown'} />
                <Row label='Phone Number ID' value={details?.wabaLifecycle.phoneNumberId ?? 'Not available'} />
                <Row
                  label='Business Account ID'
                  value={details?.wabaLifecycle.businessAccountId ?? 'Not available'}
                />
              </Section>
                ...
                {selectedConversation.notes.length === 0 ? (
                  ...
                  <div className='space-y-3'>
                    {selectedConversation.notes.map((note) => (
                      <div key={note.id} className='bg-muted/40 rounded-xl p-3 text-sm'>
                        <div className='flex items-center justify-between gap-3'>
                          <span className='font-medium'>{note.authorDisplayName}</span>
              ...
              <Section title='Outbound Queue'>
                {selectedConversation.queue.length === 0 ? (
                  <p className='text-muted-foreground text-sm'>No queued outbound jobs.</p>
                ) : (
                  <div className='space-y-3'>
                    {selectedConversation.queue.map((job) => (
                      <div key={job.id} className='bg-muted/40 rounded-xl p-3 text-sm'>
                        <div className='flex items-center justify-between gap-3'>
                          <span className='font-medium'>{job.status}</span>
                          <span className='text-muted-foreground text-xs'>{job.createdAt}</span>
                        </div>
                        <div className='mt-2 grid gap-2'>
                          ...
                          <Row label='Template' value={job.templateName ?? 'Not applicable'} />
                          <Row label='Provider Message ID' value={job.providerMessageId ?? 'Pending'} />
                          <Row label='Failure' value={job.failureMessage ?? job.failureCode ?? 'No failure recorded'} />
                        </div>
                      </div>
                    ))}
                  </div>
