import type { Conversation, ConversationDetails, ConversationMetadata, Message } from './types';
...
export function mapConversationSummary(summary: ChatWorkspaceSummaryRecord): Conversation {
  ...
  return {
    ...
    status: summary.status === 'open' && summary.serviceWindowOpen ? 'online' : 'offline',
    unread: 0,
...
export function mapSelectedConversation(
  ...
  const name = selectedConversation.profileName ?? selectedConversation.waId ?? 'Unknown';
  ...
  return {
    ...
    status:
      selectedConversation.status === 'open' && selectedConversation.serviceWindowOpen
        ? 'online'
        : 'offline',
    unread: 0,
    initials: getInitials(name),
...
export function mapConversationDetails(details: DrawerDetailsRecord | undefined): ConversationDetails | null {
  if (!details) {
    return null;
  }

  const selectedConversation: ConversationMetadata | null = details.selectedConversation
    ? {
        ...
        serviceWindowExpiresAt: details.selectedConversation.serviceWindowExpiresAt
          ? formatTimestamp(details.selectedConversation.serviceWindowExpiresAt)
          : null,
        serviceWindowExpiringSoon: details.selectedConversation.serviceWindowExpiringSoon,
        serviceWindowOpen: details.selectedConversation.serviceWindowOpen,
        lastInboundAt: formatTimestamp(details.selectedConversation.lastInboundAt),
        lastMessageAt: formatTimestamp(details.selectedConversation.lastMessageAt),
