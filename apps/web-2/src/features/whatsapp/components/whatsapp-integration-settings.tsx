...
function getTemplateBody(
  components: Array<{ type?: string } & Record<string, unknown>>,
) {
  return (
    (
      components.find((component) => component.type === "BODY") as
        | { text?: string }
        | undefined
    )?.text ?? ""
  );
...
export function WhatsAppIntegrationSettings({
  ...
  const integrationState = useQuery(
    api.whatsapp.getWhatsAppIntegrationState,
    {},
  );
  const saveWhatsAppIntegration = useMutation(
    api.whatsapp.saveWhatsAppIntegration,
  );
  const saveWhatsAppTemplate = useMutation(api.whatsapp.saveWhatsAppTemplate);
  const archiveWhatsAppTemplate = useMutation(
    api.whatsapp.archiveWhatsAppTemplate,
  );
  ...
  const syncWhatsAppTemplates = useAction(
    api.whatsappAction.syncWhatsAppTemplates,
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(
        `WhatsApp integration saved (${normalizeStatusLabel(result.connectionStatus)}).`,
      );
      setForm((current) => ({
        ...current,
        accessToken: "",
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-muted-foreground text-sm">
                Connect the active bot to your WhatsApp Business credentials and
                webhook.
              </p>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      handleFieldChange("appSecret", event.target.value)
                    }
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      handleFieldChange("verifyToken", event.target.value)
                    }
                  />
                </div>
              </div>
              ...
              <div className="flex items-center justify-between rounded-lg border px-4 py-3">
                <div className="space-y-1">
                  ...
                  <p className="text-muted-foreground text-sm">
                    Turn on message transport after the credentials are
                    configured.
                  </p>
                </div>
                <Switch
            ...
            <CardHeader>
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-muted-foreground text-sm">
                    Keep reply templates ready for conversations outside the
                    service window.
                  </p>
                </div>
                <Button
              ...
              <div className="space-y-3">
                {integrationState && integrationState.templates.length === 0 ? (
                  <div className="rounded-lg border px-4 py-5 text-sm">
                    No templates yet. Create or sync templates before sending
                    after the service window closes.
                  </div>
                ) : null}

                {integrationState?.templates.map(
                  (template: {
                    id: string;
                    name: string;
                    languageCode: string;
                        ...
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button
                              ...
                              variant="outline"
                              onClick={() =>
                                handleTemplateArchive(String(template.id))
                              }
                            >
                              Archive
                            </Button>
              ...
              {isLoadingState ? (
                ...
                <>
                  <div className="flex flex-wrap gap-2">
                    <Badge
                      variant={getStatusBadgeVariant(state.connectionStatus)}
                    >
                  ...
                  <Row
                    label="Linked Bot"
                    value={linkedBotName ?? "Not configured"}
                  />
                  <Row
                    label="Webhook Status"
                    value={normalizeStatusLabel(state.webhookStatus)}
                  />
                  <Row
                    label="Last Webhook Event"
                    value={formatTimestamp(state.lastWebhookEventAt)}
                  />
                  <Row
            ...
            <CardContent className="space-y-3">
              {isLoadingState ? (
                ...
                <>
                  <Row
                    label="Access Token"
                    value={state.hasAccessToken ? "Configured" : "Missing"}
                  />
                  <Row
                    label="App Secret"
                    value={state.hasAppSecret ? "Configured" : "Missing"}
                  />
                  <Row
                    label="Verify Token"
                    value={state.hasVerifyToken ? "Configured" : "Missing"}
                  />
                </>
              )}
            </CardContent>
              ...
              {isLoadingState ? (
                ...
                <>
                  <div className="grid gap-3">
                    <Row
                      label="Approval"
                      value={normalizeStatusLabel(state.approvalStatus)}
                    />
                    <Row
                    ...
                    <Row
                      ...
                      value={normalizeStatusLabel(
                        state.displayNameReviewStatus,
                      )}
                    />
                    <Row
                      label="Messaging Tier"
                      value={normalizeStatusLabel(
                        state.messagingLimitTier ?? "not_available",
                      )}
                    />
                  </div>
