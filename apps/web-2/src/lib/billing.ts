...
export function isSupportedGateway(gateway: string): gateway is BillingGateway {
  return gateway === 'polar' || gateway === 'midtrans';
}
...
function parseMidtransOrderId(orderId?: string | null) {
...
function parseProviderTimestamp(value?: string | null) {
  if (!value) {
    return undefined;
  }
...
function addOneMonth(timestamp: number) {
...
export function normalizeMidtransBillingWebhook(payload: Record<string, unknown>) {
  ...
  const clerkOrgId =
    typeof payload.custom_field1 === 'string' ? payload.custom_field1 : parsedOrder?.clerkOrgId;
  const planKey =
    typeof payload.custom_field2 === 'string' ? payload.custom_field2 : parsedOrder?.planKey;
  ...
  const currentPeriodStart =
    transactionStatus.toLowerCase() === 'capture' || transactionStatus.toLowerCase() === 'settlement'
      ? transactionTimestamp
      : undefined;
  const currentPeriodEnd = currentPeriodStart ? addOneMonth(currentPeriodStart) - 1 : undefined;
...
export async function forwardBillingEventToConvex(payload: Record<string, unknown>) {
  ...
  const response = await fetch(`${convexHttpUrl.replace(/\/$/, '')}/internal/billing/webhook-events`, {
    method: 'POST',
    headers: {
