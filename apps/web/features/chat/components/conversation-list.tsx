...
export function ConversationList({
  ...
  const filtered = useMemo(() => {
    ...
    return conversations.filter((conversation) => conversation.searchText.includes(q));
  ...
  return (
    <div className="border-border/40 bg-background/75 hidden h-full flex-col gap-4 overflow-hidden rounded-2xl border p-3 backdrop-blur lg:col-start-1 lg:col-end-2 lg:flex lg:rounded-3xl lg:p-4">
      ...
      <div className="relative">
        <Icons.search
          className="text-muted-foreground/70 pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2"
          aria-hidden="true"
        />
        <Input
          id="messenger-search"
      ...
      <div className="flex-1 space-y-2 overflow-y-auto pr-1" aria-label="Conversation list" role="list">
        {filtered.length === 0 ? (
          ...
          return (
            ...
            >
              <div className="relative shrink-0">
                ...
                <span
                  className={cn(
                    "border-background absolute bottom-0 right-0 inline-flex h-3 w-3 rounded-full border-2",
